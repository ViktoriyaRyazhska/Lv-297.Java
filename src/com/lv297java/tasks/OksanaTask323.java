package com.lv297java.tasks;

import com.lv297java.AbstractTask;

import java.util.ArrayList;
import java.util.List;
import java.util.Scanner;

/**
 * This class implement task 323:
 * <p>Current task is to find all natural numbers
 * that are less than n and are mutually simple with it.</p>
 *
 * @version 1.0
 * @author Oksana Mekh
 */
public class OksanaTask323 extends AbstractTask {
    /**
     * Initializes a newly created {@link OksanaTask323} object.
     * Constructor without parameters exist for initialise field
     * {@link AbstractTask#testID} in parent {@code AbstractTest} Class
     It represents a Task 323.
     */
    public OksanaTask323() {
        super("323");
    }

    /**
     * Finds GCD of two numbers.
     * @param a first number
     * @param b second number
     * @return GCD of two numbers
     */
    public static int greatestCommonDivisor(final int a, final int b) {
        if (b == 0) {
            return a;
        }
        int remainder = a % b;
        return greatestCommonDivisor(b, remainder);
    }

    /**
     * Finds all natural numbers that are less than
     * {@code n} and are mutually simple with {@code n}.
     *
     * @param n natural number
     * @return list of natural numbers
     */
    public static List<Integer> findAllResultNumbers(int n) {
        List<Integer> resultNumbers = new ArrayList<>();
        for (int i = 1; i < n; i++) {
            if (greatestCommonDivisor(i, n) == 1) {
                resultNumbers.add(i);
            }
        }
        return resultNumbers;
    }

    /**
     * Read natural number from console input.
     * @return natural number
     */
    public static int readNumber() {
        Scanner scanner = new Scanner(System.in);
        while (true) {
            System.out.print("Enter number: ");
            if (scanner.hasNextInt()) {
                int n = scanner.nextInt();
                if (n < 1) {
                    System.out.println("Number can not be less then 1.");
                } else {
                    return n;
                }
            } else {
                System.out.println("Input text is not a number.");
            }
        }
    }

    /**
     * {@inheritDoc}
     * <p>This method finds all natural numbers
     * that are less than n and are mutually simple with it.</p>
     * <p>It expects the input of one number.</p>
     */
    @Override
    public void execute() {
        int n = readNumber();

        System.out.println("Result: " + findAllResultNumbers(n));
        System.out.println("Task executed.");
    }
}
